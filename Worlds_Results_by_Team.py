...
worlds_list = ['RE-VRC-11-5134', 'RE-VRC-14-0110', 'RE-VRC-15-1717', 'RE-VRC-16-3279', 'RE-VRC-17-4887', 'RE-VRC-17-3805',
               'RE-VRC-18-6082', 'RE-VRC-19-8379']
...
for world in worlds_list:
    for team in team_list:
        with urlopen(f'https://api.vexdb.io/v1/get_events?team={team}&sku={world}') as resp:
            db = resp.read()
            db_data = json.loads(db)
            data = (db_data['result'])
            df_temp = pd.DataFrame.from_dict(data)
...
# with pd.ExcelWriter('Worlds_results_team.xlsx') as writer:  # pylint: disable=abstract-class-instantiated
...
# with pd.ExcelWriter('Worlds_tables_team.xlsx') as writer:  # pylint: disable=abstract-class-instantiated
